...
public class MovieProgram {
    ...
    private Movie[] fillMoviesArray(Movie[] films) {
    ...
    private Movie getMostRecentMovie(Movie[] films) {
        ...
        Movie mostRecentMovie = films[0];
        for (int i=0; i < films.length; i++){
            if(films[i].isMoreRecentThan(mostRecentMovie)){
                mostRecentMovie = films[i];
            }
        }
    ...
    private Movie getLongestMovie(Movie[] movies) {
        ...
        for (int i=0; i < movies.length; i++){
            if (movies[i].isLongerThan(longestMovie)){
    ...
    private void printDirector(String movieName, Movie[] movies) {
        ...
        while(i < movies.length){
            if (movieName == movies[i].getName()){
                System.out.println(movieName + " was directed by " + movies[i].getDirector() + ".");
            }
            i++;
        }
        if (i == movies.length && movieName.equals(movies[i-1].getName())) {
